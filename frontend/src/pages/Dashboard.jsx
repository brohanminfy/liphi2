            ...
            <>
              ...
              {(() => {
                ...
                documents.forEach((doc) => {
                  ...
                  if (userRole === 'admin') adminDocs.push(doc);
                  else if (userRole === 'editor') editorDocs.push(doc);
                  else if (userRole === 'viewer') viewerDocs.push(doc);
                });
                return (
                  <>
                    {adminDocs.length > 0 && (
                      <div className="mb-4">
                        <h4 className="text-xs font-semibold text-yellow-700 uppercase mb-2 tracking-wider">Admin</h4>
                    ...
                    {editorDocs.length > 0 && (
                      <div className="mb-4">
                        <h4 className="text-xs font-semibold text-blue-700 uppercase mb-2 tracking-wider">Editor</h4>
                        <ul className="space-y-1">
                          {editorDocs.map((doc) => {
                            const roleInfo = getRoleInfo('editor');
                            return (
                              <li
                                key={doc.id}
                                onClick={() => setSelectedDocId(doc.id)}
                    ...
                    {viewerDocs.length > 0 && (
                      <div className="mb-4">
                        <h4 className="text-xs font-semibold text-gray-700 uppercase mb-2 tracking-wider">Viewer</h4>
                        <ul className="space-y-1">
                          {viewerDocs.map((doc) => {
                            const roleInfo = getRoleInfo('viewer');
                            return (
                              <li
                                key={doc.id}
                                onClick={() => setSelectedDocId(doc.id)}
        ...
        <main className="flex-1 p-0 sm:p-4 overflow-y-auto relative min-w-0">
          {selectedDocId ? (
            <YDocProvider docId={selectedDocId} authEndpoint="https://demos.y-sweet.dev/api/auth" showDebuggerLink={false}>
              <DocumentEditor documentId={selectedDocId} onTitleUpdate={handleDocumentTitleUpdate} suppressLoading={false} />
            </YDocProvider>
          ) : (
            <div className="flex items-center justify-center h-full text-gray-400 text-lg">Select a document to start editing</div>
          )}
        </main>
      </div>
    </div>
